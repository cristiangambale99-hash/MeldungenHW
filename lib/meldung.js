...
// Bereiche, bei denen das Stockwerk angegeben werden muss
const MIT_STOCKWERK = ['mieter', 'treppenhaus'];
...
export function bereinige(e) {
  ...
  if (!intern) {
    ...
    // Stockwerk nur dort, wo es Sinn macht (Mieter, Treppenhaus)
    m.bereich = BEREICHE[e.bereich] ? e.bereich : null;
    if (MIT_STOCKWERK.includes(m.ort) && !m.bereich) throw fehler('bereich');
    if (!MIT_STOCKWERK.includes(m.ort)) m.bereich = null;
